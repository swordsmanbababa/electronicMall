...
//			generation=sort_select(generation);  //排序后筛选 将排名靠前的作为下一代父体
	...
	public List<Genents> sort(List<Genents> genList){
		...
		int i, j;
		int gen[][] = new int[genList.size()][2];
		for (i = 0; i < gen.length; i++) {
			gen[i][0] = i;
			gen[i][1] = Operator.getShiftCount(root, genList.get(i));
		}
		// 按照迁移次数排序
		for (i = 0; i < gen.length; i++) {
		...
		// System.out.println("基因排序后结果：\n");
		if (genList.get(gen[0][0]).getIs_init() == 1) { // 说明是父个体
			for (i = 0; i < CommonConstants.ProgenyCount_num; i++) {
				generation_new.add(genList.get(gen[i][0]));
			}
		} else {
			for (i = 1; i < CommonConstants.ProgenyCount_num + 1; i++) {
				generation_new.add(genList.get(gen[i][0]));
			}
		}
